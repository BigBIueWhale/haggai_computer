...
with an explicit port set for each desktop.
...
> ⚠️ Each desktop publishes RustDesk (`21128/tcp`) and the explicit web-preview
> ports `3000/tcp`, `5173/tcp`, and `8080/tcp` on your DMZ'd box. T3 Code's
> `3773/tcp` is bound to host loopback only. Add the public ports to the allow-list
> in [`docs/SECURITY.md`](docs/SECURITY.md) so the audit stays truthful.
...
- **T3 Code** appears in Plasma's application menu like the other GUI apps. The
  `t3` CLI is also installed; to use its web UI through the loopback-only port:
  ```fish
  t3 --host 0.0.0.0 --port 3773 --no-browser
  ```
...
`./home` starts empty) its own name, port set, and home directory, then run its
`setup.sh`:
...
`docs/SECURITY.md` for every public port.
